...
version: v1.1
updated: 2026-04-02
portal: https://lrndxihi.gensparkclaw.com/benny/10_API명세서_기술사양서.html
---

10_API명세서_기술사양서 — 소울나루
...
### 4.1 인앱결제 상품 ID (2026-04-03 업데이트)
...
| 프리미엄 구독 (연) | `com.soulnaru.premium.yearly` | `com.soulnaru.premium.yearly` |


#### 시즌패스 상품 (봄→런칭+여름으로 전환)


| 상품 | Android | iOS |

| 런칭 시즌패스 (한정) | `com.soulnaru.season.launch2026` | `com.soulnaru.season.launch2026` |

| 여름 시즌패스 | `com.soulnaru.season.summer2026` | `com.soulnaru.season.summer2026` |

| 가을 시즌패스 | `com.soulnaru.season.autumn2026` | `com.soulnaru.season.autumn2026` |


#### 씨앗(🌱) 소액 충전 상품 (신규)


| 상품명 | 씨앗 수량 | Android | iOS |

| 씨앗 한 줌 | 300개 | `com.soulnaru.seeds.small` | `com.soulnaru.seeds.small` |

| 씨앗 한 봉지 | 800개 | `com.soulnaru.seeds.medium` | `com.soulnaru.seeds.medium` |

| 씨앗 한 바구니 | 2,000개 | `com.soulnaru.seeds.large` | `com.soulnaru.seeds.large` |

| 씨앗 한 자루 | 5,500개 | `com.soulnaru.seeds.xlarge` | `com.soulnaru.seeds.xlarge` |
...
POST /payments/verify
  Body: { "platform": "android", "receipt": "...", "product_id": "com.soulnaru.premium.monthly" }
    ↓
[서버] Google/Apple 서버 검증
    ↓
Response: { "success": true, "subscription": { "type": "premium", "expires_at": "..." } }
`
```
...
[클라이언트] Unity IAP 구매 → Receipt 획득
    ↓
POST /payments/seeds/purchase
  Body: {
    "platform": "android",
    "receipt": "...",
    "product_id": "com.soulnaru.seeds.medium"
  }
    ↓
[서버] Google/Apple 서버 검증 → 영수증 유효 확인
    ↓
[서버] 유저 씨앗 잔고 += 800 (상품별 수량)
    ↓
Response: {
  "success": true,
  "seeds_added": 800,
...
| `SEEDS_001` | 400 | 영수증 검증 실패 | 재시도 안내 |

| `SEEDS_002` | 409 | 중복 영수증 (이미 처리됨) | 잔고 확인 안내 |

| `SEEDS_003` | 500 | 씨앗 지급 실패 | 고객센터 안내 + 자동 재처리 큐 |
...
(v1.1 변경: VARCO 엔드포인트 경고 추가, 구독 취소 웹훅 추가)
(v1.2 변경: ①봄→런칭+여름 시즌패스 상품 ID 전환, ②씨앗 충전 인앱결제 상품 ID + 검증 API + 에러코드 추가)