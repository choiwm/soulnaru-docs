...
48번 — 아트팀 Sprint 2 진행 추적 보고서 · 소울나루
  ...
  담당: 아티스트 A(UI) · B(2D 이미지) · C(스프라이트) · D(UI UI 프리팹) · E(파티클)
...
## 1. Sprint 2 아트 작업 현황
  ...
  | Unity UI UI 프리팹 — 3단계 Bloom | 아티스트 D | 4/17 | PNG 스프라이트 대기 | 아티스트 B PNG 스프라이트 수령 후 착수 |

  | Unity UI UI 프리팹 — 4단계 Fruit | 아티스트 D | 4/18 | PNG 스프라이트 대기 |  |

  | Unity UI UI 프리팹 — 5단계 Tree | 아티스트 D | 4/18 | PNG 스프라이트 대기 | BennySeasonManager 연결 포함 |
...
## 4. 파티클 작업 지침 (아티스트 E 참조)
  ...
  UI UI 프리팹 조립(아티스트 D), Unity 임포트(개발팀) 일정이 연쇄 지연됩니다. Alpha 빌드(5/14) 리스크 상승.
