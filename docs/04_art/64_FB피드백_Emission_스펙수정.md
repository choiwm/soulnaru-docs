...
# ✨ FB-11: 베니 5단계 Highlight Intensity 0.3으로 조정 (41번 업데이트)
 ...
 **작성일:** 2026-04-28
 ...
 ✅ **FB-11 완료** — Benny_S5_Highlight.png 발광 강도 HDR 1.0 → 0.3으로 감소. 과도한 밝기 제거, 자연스러운 발광 표현.
 

 
## 1. 문제 분석

 **테스트 결과:** 베니 5단계 나무 에셋의 Highlight 이미지 레이어가 일반 실내 조명에서 과도하게 밝게 표현됨. 플레이어 경험상 불편하고, 게임의 시각적 톤과 맞지 않음. 
 **원인:** Highlight Intensity가 HDR 스케일 1.0 설정 → 현재 라이팅 환경에서 과다 노출 
...
### 이미지 레이어 Highlight 스펙 변경

 
 | 항목 | 기존 값 | 신규 값 | 변경사항 |

 | Highlight Map | Benny_S5_Highlight.png | Benny_S5_Highlight.png | 변경 없음 |

 | Highlight Intensity (HDR) | 1.0 | 0.3 | ✅ 감소 |

 | Highlight Color | #DDD6FE (라벤더 흰색) | #DDD6FE | 변경 없음 |
...
Material Properties:
 ...
 - BaseColor: [Benny_S5_BaseColor]
 - Shadow Map: [Benny_S5_Shadow]
 - Highlight: [Benny_S5_Highlight]
 
// 수정 항목
 - Highlight Intensity: 1.0 → 0.3 ✅
 - Highlight Color: #DDD6FE (유지) 

 **Step 2: Inspector 설정 (Unity Editor)** 
 1. Project → Assets → Benny_S5.mat 선택
2. Inspector → Highlight (드롭다운)
...
## 5. QA 검증 기준 (4/30)
 ...
 | Highlight 감소 확인 | HDR 0.3 적용 후 시각적 밝기 30% 이상 감소 | 확인 필요 |

 | 시각적 톤 일관성 | 1~4단계 나무와 비교했을 때 과도하지 않은 수준 | 확인 필요 |

 | 라벤더 색감 유지 | Highlight Color는 #DDD6FE로 유지 — 색상 손실 없음 확인 필요 |
...
## 7. 문서 업데이트 (41번 스펙서)
 ...
 ## 3. 이미지 레이어 색상 스펙 — 5단계 나무 (기존)
Highlight: #DDD6FE + 계절별 추가 발광

→ 수정됨 (v1.1):

## 3. 이미지 레이어 색상 스펙 — 5단계 나무 ✅
Highlight: #DDD6FE + Intensity HDR 0.3 (과도한 밝기 제거)
- 기존: HDR 1.0 (과도한 발광)
